 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`jincore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`jincore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'jincore-' + name : 'jincore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/jincore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:' + fullname + ' --external jincore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:jincore-lib -o jincore-lib.js';
    }
